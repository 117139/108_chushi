<template>
	<view class="release">
		<!-- 发布 -->
		<view v-if="datas" class="header_top area">
		...
		<view v-if="datas" class="user_news_wrap area">
			...
			getdatas(){
				...
				that.$service.P_post(jkurl, datas,header).then(res => {
					...
					if (res.code == 1) {
						...
						that.img_arr=datas.img_arr
						that.phone=datas.phone
						that.is_top=datas.is_top
						that.address={
						...
						for (var i = 0; i < that.sortList.length; i++) {
							if(that.sortList[i].id==datas.pid){
								that.current=i
							}
						}
					} else {
